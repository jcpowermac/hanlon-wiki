# Using Occam to Provision ESXi 

### Load the VMware-VMvisor-Installer-5.0.0-469512.x86_64.iso into Occam image service.
    $ occam image add [os|mk|esxi] (PATH TO ISO)

    $ occam image add esxi VMware-VMvisor-Installer-5.0.0-469512.x86_64.iso
    ...
    Images:
            ...
            Path: /root/macshared/occam/image/esxi/4fhYStNwgcjUMKHvFIChSy  
            Status: Valid   
...
When asked, use the example AAAAA-BBBBB-CCCCC-DDDDD-EEEEE as the ESX License Key.
    ...
    $ bin/occam model get template
    Valid Model Templates:
            ubuntu_oneiric_min  :  Ubuntu Oneiric 11.10 Minimal
            vmware_esxi5_simple  :  VMware ESXi 5 Simple Deployment

    $ occam model add (model_template) (model name) {image_uuid}

    $ occam model add vmware_esxi5_simple esxi_model 4fhYStNwgcjUMKHvFIChSy
...
### Use the `vmware_hypervisor` template to create a policy that connects tags with the model.
    ...
    $ occam policy add (policy templates) (Name) (Model Config UUID) [none|(Broker Target UUID)] (tag{,tag,tag})

    $ occam policy add vmware_hypervisor esxi_policy 3FzmV4X8l53CWHu3iZC0DQ none vmware_vm
